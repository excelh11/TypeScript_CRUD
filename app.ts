/**
 * 1. 타입 정의 (Alias, Literal, Intersection)
 */
type Priority = "High" | "Medium" | "Low";
type Point = 20 | 30 | 40 | 50;
type StatusMessage = "Success" | "Error" | "Warning";
 ...
 */
function throwSystemException(message: string): never {
 ...
 * 3. 상태 로그 함수 (void 활용)
 */
function logStatus(msg: string, type: StatusMessage = "Success"): void {
 ...
 */
function calculateBonus(base: Point): number {
    return base * 2;
}
...
(window as any).completeTodo = (id: number) => {
    const todo = todos.find(t => t.id === id);
